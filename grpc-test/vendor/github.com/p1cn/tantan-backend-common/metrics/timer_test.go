...
func TestTimer(t *testing.T) {
	...
	timer := NewTimer("tantan", "http_request", "help", []string{"http_code", "url"})
...
func mockHttpHandleTime(timer *Timer, t *testing.T) {
	...
	// 结束计时，
	tf("200", "http://tantan.com/users")

	// 在终端打印出promtheus指标
	// 将看见前缀为 : tantan_http_request_h_*  和 tantan_http_request_s* 的指标
...
func mockHttpHandleObserve(timer *Timer, t *testing.T) {
	...
	timer.Observe(time.Now().Sub(startTime), "200", "http://tantan.com/users")
